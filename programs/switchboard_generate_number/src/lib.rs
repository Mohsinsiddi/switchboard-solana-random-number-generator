...
declare_id!("5p3k3TUCfwYof4HLVsjDgLcWv21YxkPccHMJCXX4AEA2");


#[program]
pub mod switchboard_generate_number {
    use super::*;
    ...
    pub fn coin_flip(ctx: Context<CoinFlip>, randomness_account: Pubkey, guess: bool) -> Result<()> {
        msg!("Coin Flipping By Md Mohsin Siddiqui");
        let clock = Clock::get()?;
        let player_state = &mut ctx.accounts.player_state;
    ...
    pub fn settle_flip(ctx: Context<SettleFlip>, escrow_bump: u8) -> Result<()> {
        msg!("Coin Settle Fliiping Result By Md Mohsin Siddiqui");
        ...
        msg!("Random Value Generated : {}",revealed_random_value[0]);
        let randomness_result: bool = revealed_random_value[0] % 2 == 0;
